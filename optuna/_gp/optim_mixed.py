...
from optuna._gp.acqf_numba import eval_acqf_with_grad, eval_acqf_no_grad
from optuna._gp.search_space import ScaleType, sample_normalized_params

CATEGORICAL = int(ScaleType.CATEGORICAL)
LOG = int(ScaleType.LOG)

@numba.njit
def local_search(acqf_params: AcquisitionFunctionParams, x0: np.ndarray, tol: float) -> tuple[np.ndarray, float]:
    grad_scale = 1 / acqf_params.kernel_params.inverse_squared_lengthscales
    param_types, bounds, steps = acqf_params.search_space

    def fun(x: np.ndarray) -> float:
        return eval_acqf_no_grad(acqf_params, x)
    def fun_with_grad(x: np.ndarray) -> tuple[float, np.ndarray]:
        return eval_acqf_with_grad(acqf_params, x)
