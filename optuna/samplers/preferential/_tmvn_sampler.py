...
class TruncatedMVN:
    ...
    def sample(self, n, oversampling_rate=10):
        """
        ...
        while accept < n:
            logpr, Z = self.mvnrnd(int(n * oversampling_rate), self.mu)  # simulate n proposals
            idx = -np.log(self.random_state.rand(int(n * oversampling_rate))) > (self.psistar - logpr)  # acceptance tests
